...
Phase 1 appliance / lab captures for SLS Camera firmware.  
**Canonical location:** repo root `screenshots/` (not `docs/images/`).

Captured from KVM guest **`sls-appliance-phase1`** (Lubuntu **26.04**) and field notes:
...
```bash
# guest, as sls
DISPLAY=:0 /usr/local/bin/sls-camera --demo    # UI without Kinect
DISPLAY=:0 /usr/local/bin/sls-camera           # live after: ./scripts/vm-kinect-usb.sh reattach
```
...
| [20260722-screen.jpg](20260722-screen.jpg) | **2026-07-22** lab capture (operator-added) |

**Guest resources for 10–13:** tablet-class **2 GiB** — [docs/VM-REBUILD.md](../docs/VM-REBUILD.md).
...
# host
virsh -c qemu:///system screenshot sls-appliance-phase1 screenshots/out.png
```

Isochronous USB can glitch under SPICE; bare metal remains field truth. See [docs/POWER-AND-DISPLAY.md](../docs/POWER-AND-DISPLAY.md).
